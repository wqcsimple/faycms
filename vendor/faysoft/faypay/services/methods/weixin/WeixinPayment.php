...
class WeixinPayment implements PaymentMethodInterface{
    ...
    public function jsapi(PaymentTradeModel $trade, PaymentMethodConfigModel $config){
        ...
        \WxPayConfig::$APPID = $config->app_id;
        \WxPayConfig::$APPSECRET = $config->app_secret;
        \WxPayConfig::$MCHID = $config->mch_id;
        
        //①、获取用户openid
        $tools = new \JsApiPay($config->app_id, $config->app_secret);
        if(\F::app()->current_user && $openId = UserOauthService::service()->getOpenId($config->app_id)){
